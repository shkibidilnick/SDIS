...
void SafetyTraining::certifyEmployee(const std::string& employeeId) {
  ...
  auto expirationDate =
      now + std::chrono::hours(24 * 30 * validityPeriodMonths);
  employeeExpirations[employeeId] = expirationDate;
...
void SafetyTraining::revokeCertification(const std::string& employeeId) {
  auto it = std::find(employeeCertifications.begin(),
  ...
  employeeCertifications.erase(it);

  employeeExpirations.erase(employeeId);
...
bool SafetyTraining::isCertificationExpired(
  ...
  auto it = employeeExpirations.find(employeeId);
  if (it == employeeExpirations.end()) {
    throw SafetyException("No expiration date found for employee " +
                          employeeId);
  }
